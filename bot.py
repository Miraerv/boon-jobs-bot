...
async def branch(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "В каком графике ты хотел(-а) бы работать в ближайшее время?",
...
async def salary_expect(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if position == "сборщик":
        await update.message.reply_text(
            ...
            "* Взаимодействие с поддержкой по заказам\n"
            "* Прием оплаты\n\n"
            "⚡ Что важно для этой работы?\n"
            "Внимательность, скорость, ответственность, аккуратность, умение работать в команде, "
            ...
            "🛠 Стажировка: 10 часов, оплачивается по 100 ₽/час.\n\n"
        ...
        await update.message.reply_text(
            ...
            "Невыход на смену без согласования влечёт удержание из зарплаты.\n\n"
            "💰 Вознаграждение:\n"
            "- 1 слой (радиус до 500 м от склада) — 100₽/заказ\n"
            "- 2 слой (радиус до 1000 м от склада) — 120₽/заказ\n\n"
...
async def vacancy_info(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.text.strip() == "+":
        await update.message.reply_text(
            "Спасибо за заявку! Менеджер получит твои ответы и свяжется для получения дополнительной информации "
            "в течение 1–2 рабочих дней.\n\n"
            "Также менеджер поможет тебе поскорее начать стажировку и стать частью команды Boon Express 🚀\n\n"
            "До скорого! 💜"
        )
