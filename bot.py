...
async def salary_expect(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if position == "сборщик":
        ...
        await update.message.reply_text(
            ...
            "\n\nВознаграждение:\n"
            "- 1 слой (до 500 м) — 85₽/заказ\n"
            "- 2 слой (до 1000 м) — 100₽/заказ\n\n"
            "🛵 Стажировка: 1 день + инструктаж по электровелосипеду.\n"
            "Электровелосипед выдается в аренду — удержание 13% от недельного дохода.\n\n"
            "💳 Выплаты: еженедельно (вт–ср) на карту.\n\n"
            "Ты готов присоединиться к нашей команде?\nЕсли всё устраивает — поставь + 😊"
        )
    return VACANCY_INFO
